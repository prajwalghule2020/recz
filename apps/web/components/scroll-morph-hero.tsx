...
const SCATTER_END_SCROLL = 120;
const LINE_END_SCROLL = 320;
...
export default function IntroAnimation({ className = "" }: IntroAnimationProps) {
    ...
    const currentPhaseRef = useRef<AnimationPhase>("scatter");
    ...
    useEffect(() => {
        ...
        const updateInView = () => {
            ...
            // Lock only when the reveal block is fully framed in the viewport,
            // not when it just peeks in at the bottom of the hero.
            const topFrame = 110; // keep space for sticky navbar
            const bottomFrame = 24;
            const isFramed = rect.top <= topFrame && rect.top >= topFrame - 70 && rect.bottom <= viewportHeight - bottomFrame;

            setIsInView(isFramed);
        };
    ...
    const shouldLockScroll = isInView && !isRevealComplete;
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            const keyDeltaMap: Record<string, number> = {
                ArrowDown: 80,
                ArrowUp: -80,
                PageDown: 240,
                PageUp: -240,
        ...
        // Capture scroll input globally while reveal is locked.
    ...
    }, [shouldLockScroll, virtualScroll]);

    useEffect(() => {
        const unsubscribe = virtualScroll.on("change", (value) => {
            let nextPhase: AnimationPhase = "circle";

            if (value < SCATTER_END_SCROLL) {
                nextPhase = "scatter";
            } else if (value < LINE_END_SCROLL) {
                nextPhase = "line";
            }
    ...
    // Keep the page pinned until the morph reveal has completed.
    useEffect(() => {
    ...
    // Mark reveal complete only when morph is visually at the final state.
    useEffect(() => {
    ...
    const isIntroVisible = introPhase !== "circle" || morphValue < 0.65;
    const introFade = introPhase === "circle" ? Math.max(0, 1 - morphValue / 0.65) : 1;
    ...
    return (
        <div ref={containerRef} className={`relative h-full w-full overflow-hidden ${className}`}>
            ...
            <div className="flex h-full w-full flex-col items-center justify-center perspective-1000">
                ...
                <div className="pointer-events-none absolute top-1/2 z-20 flex -translate-y-1/2 flex-col items-center justify-center text-center">
                    <motion.div
                        initial={{ opacity: 0, scale: 0.96 }}
                        animate={isIntroVisible ? { opacity: introFade, scale: 1 } : { opacity: 0, scale: 0.98 }}
                        transition={{ duration: 0.8 }}
                        className="absolute h-[210px] w-[210px] rounded-full bg-[#0b1222]/95 shadow-[0_20px_60px_-30px_rgba(0,0,0,0.8)] md:h-[280px] md:w-[280px]"
                    />
                    <motion.h1
                        initial={{ opacity: 0, y: 20, filter: "blur(10px)" }}
                        animate={isIntroVisible ? { opacity: introFade, y: 0, filter: "blur(0px)" } : { opacity: 0, filter: "blur(10px)" }}
                        transition={{ duration: 1 }}
                        className="relative px-4 text-2xl font-semibold tracking-tight text-white md:text-4xl"
                    >
                        <span className="block">The future is</span>
                        <span className="block">built on AI.</span>
                    </motion.h1>
                    <motion.p
                        initial={{ opacity: 0 }}
                        animate={isIntroVisible ? { opacity: introFade * 0.55 } : { opacity: 0 }}
                        transition={{ duration: 1, delay: 0.2 }}
                        className="relative mt-4 text-xs font-bold tracking-[0.2em] text-white/55"
                    >
                        SCROLL TO EXPLORE
                    </motion.p>
                </div>
                ...
                >
                    <h2 className="mb-4 text-3xl font-semibold tracking-tight text-title dark:text-white md:text-5xl">
                        Turn Every Photo Into Searchable Intelligence
                    </h2>
                    <p className="max-w-lg text-sm leading-relaxed text-paragraph dark:text-paragraph-dark md:text-base">
                        Face AI detects, clusters, and organizes<br/> people across your photo library <br/>automatically. <br className="hidden md:block" />
                        Scroll to explore magic
